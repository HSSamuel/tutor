...
// --- 1. FONTS IMPORT ---
// Added 'Caladea' (The Google Font equivalent of Cambria)
import { Outfit, JetBrains_Mono, Tinos, Caladea } from "next/font/google";

// Main App Font (UI Elements)
const mainFont = Outfit({
  subsets: ["latin"],
  weight: ["300", "400", "500", "700", "800"],
});

// UI Label Font (Tech/Code style)
...
export default function Home() {
  ...
  const saveToHistory = async (topic: string) => {
    ...
    // Check for duplicates to avoid spamming history
    const isDuplicate =
      history.length > 0 &&
      history[0].subject.toLowerCase() === topic.toLowerCase();
  ...
  const deleteHistoryItem = async (itemId: string) => {
    ...
    if (error) {
      ...
      fetchHistory(session.user.id); // Revert if failed
    }
  };
  ...
  const handleListen = () => {
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      recognition.onresult = (event: any) =>
        setSubject(event.results[0][0].transcript);
      recognition.start();
    } else {
  ...
  const handleSpeak = (text: string) => {
    ...
    const nigerianVoice = availableVoices.find(
      (v) =>
        v.lang === "en-NG" ||
        v.name.includes("Nigeria") ||
        v.name.includes("African")
    );
  ...
  const handleSearch = async (overrideSubject?: string) => {
    ...
    try {
      ...
      setResponse(data);
      saveToHistory(topicToSearch); // <--- Auto Save Trigger
    } catch (err) {
  ...
  const checkQuiz = () => {
    ...
    const score = quizData.reduce(
      (acc, q, idx) => (q.correct === selectedAnswers[idx] ? acc + 1 : acc),
      0
    );
  ...
  return (
    <main
      className={`min-h-screen relative flex flex-col items-center justify-center p-4 md:p-6 overflow-hidden ${mainFont.className}`}
    >
      {/* NAVBAR */}
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 items-center">
          {session && (
            <div className="flex items-center gap-2 bg-gradient-to-r from-purple-900 to-slate-900 border border-purple-500/30 px-3 py-1.5 rounded-full shadow-lg">
              ...
              <div className="bg-slate-800 px-2 py-0.5 rounded text-xs font-mono text-green-400 ml-1 border border-slate-700">
                {userPoints} BP
              </div>
            </div>
          )}
          <button
            onClick={() => {
              setGriotMode(!griotMode);
              if (!griotMode) setPidginMode(false);
            }}
          ...
          >
            ...
            <span className="hidden md:inline">
              {griotMode ? "Griot: ON" : "Story Mode"}
            </span>
            <span className="md:hidden">Story</span>
          </button>
          <button
            onClick={() => {
              setPidginMode(!pidginMode);
              if (!pidginMode) setGriotMode(false);
            }}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${
              pidginMode
          ...
          >
            ...
            <span className="hidden md:inline">
              {pidginMode ? "Pidgin: ON" : "English"}
            </span>
            <span className="md:hidden">Pidgin</span>
          </button>
          {session ? (
            <>
              <button
                onClick={() => setShowHistory(!showHistory)}
                className="bg-slate-800 text-slate-300 px-3 py-2 rounded-full text-sm font-semibold flex items-center gap-2 hover:bg-slate-700 transition"
              >
                <History className="w-4 h-4" />
              </button>
              <button
                onClick={() => supabase.auth.signOut()}
                className="bg-red-500/10 text-red-400 px-3 py-2 rounded-full text-sm font-semibold flex items-center gap-2 hover:bg-red-500/20 transition"
              >
            ...
            <button
              ...
              className="bg-green-600 text-white px-4 py-1.5 rounded-full text-sm font-bold shadow-lg shadow-green-900/20 hover:bg-green-500 transition"
            >
              Sign In
            </button>
          )}
              ...
              {history.map((item) => (
                <div key={item.id} className="group flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 text-left text-sm text-slate-300 hover:text-white hover:bg-slate-800 p-2 rounded transition truncate"
                  >
                    {item.subject}
                  </button>
                  {/* DELETE BUTTON */}
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-slate-400 text-sm">
                {authMode === "signin"
                  ...
                  : "Join to start earning Brain Points!"}
              </p>
            </div>
            <form onSubmit={handleAuth} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-4 py-3 mt-1 focus:outline-none focus:border-green-500 transition-colors"
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-4 py-3 pr-12 focus:outline-none focus:border-green-500 transition-colors"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
              ...
              <button
                ...
                className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-green-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {authLoading ? (
      ...
      <div className="relative z-10 w-full max-w-2xl mt-48 md:mt-24">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl p-6 md:p-12">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm md:text-lg text-slate-400 font-light">
          ...
          <div className="relative group mb-8">
            ...
            <div className="relative flex bg-slate-900 rounded-xl p-1 items-center gap-1">
              ...
              <button
                ...
                disabled={loading}
                className="shrink-0 bg-green-600 hover:bg-green-500 text-white px-4 py-2 text-sm rounded-lg mx-1 md:px-8 md:py-3 md:text-base md:mx-0 font-semibold transition-all disabled:opacity-50 flex items-center gap-2 shadow-lg"
              >
                {loading ? (
          ...
          {error && (
            <div className="text-red-300 bg-red-900/20 p-4 rounded-xl mb-6 text-sm text-center">
              {error}
            </div>
          )}
          ...
          {response && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  ...
                  <div className="space-y-3 w-full min-w-0">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        {griotMode ? "The Tale" : "The Explanation"}
                      </h3>
                      <div className="flex gap-2">
                        <a
                          href={`https://www.youtube.com/results?search_query=${encodeURIComponent(
                            subject + " physics explanation"
                          )}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 text-xs font-bold text-red-400 hover:text-red-300 transition-colors bg-red-400/10 px-3 py-1.5 rounded-full border border-red-400/20"
                        >
                    ...
                    <div
                      className={`prose prose-invert prose-lg max-w-none text-slate-200 text-justify ${explanationFont.className}`}
                    >
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          img: ({ node, ...props }) => (
                            <div className="my-6 rounded-xl overflow-hidden border border-slate-700 shadow-lg bg-slate-900/50">
                              <img
                                {...props}
                                className="w-full h-auto object-cover hover:scale-105 transition-transform duration-500"
                                alt={props.alt || "Explanation Diagram"}
                              />
                              {props.alt && (
                                <div className="p-2 text-center text-xs text-slate-400 border-t border-slate-800 bg-slate-900/80 italic">
                                  Diagram: {props.alt}
                                </div>
                              )}
                            </div>
                    ...
                    {!quizData && (
                      <button
                        ...
                        className="w-full mt-4 py-3 bg-slate-700 hover:bg-slate-600 border border-slate-600 rounded-xl text-white font-semibold flex items-center justify-center gap-2 transition-all"
                      >
                        {quizLoading ? (
                          <Sparkles className="w-4 h-4 animate-spin" />
                        ) : (
              ...
              {quizData && (
                <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  {!showResults ? (
                    <button
                      onClick={checkQuiz}
                      className="mt-8 w-full py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-purple-900/20"
                    >
                      Submit Answers
                    </button>
                  ) : (
                    <div className="mt-8 p-6 bg-slate-800 rounded-xl text-center animate-in zoom-in-95 duration-300">
                      {(() => {
                        const score = quizData.reduce(
                          (acc, q, idx) =>
                            q.correct === selectedAnswers[idx] ? acc + 1 : acc,
                          0
                        );
                        return (
                          <>
                            ...
                            {score >= 3 ? (
                              <div className="space-y-2">
                                ...
                                {score > 0 && session && (
                                  <div className="text-purple-300 text-sm font-bold bg-purple-900/30 inline-block px-3 py-1 rounded-full border border-purple-500/30">
                                    +{score * 10} Brain Points Earned! 🚀
                                  </div>
                                )}
                              </div>
                            ) : (
              ...
              {response.visual_aid && (
                <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4 overflow-hidden">
              ...
              {/* CONTEXT CARD - TINOS (TIMES NEW ROMAN) */}
              {sourceData && (
                <div className="bg-slate-950/40 backdrop-blur-md rounded-xl p-6 border border-slate-700/50 shadow-inner group transition-all hover:bg-slate-900/60 mt-8">
                  <div className="flex items-start gap-4">
                    <div className="hidden md:flex shrink-0 p-3 bg-green-500/10 rounded-lg border border-green-500/20">
                      <MapPin className="w-5 h-5 text-green-400" />
                    </div>
                    <div className="w-full min-w-0">
                      <div className="flex items-center justify-between mb-3">
                        <span
                          className={`text-green-400 text-xs uppercase tracking-widest flex items-center gap-2 ${monoFont.className}`}
                        >
                          <MapPin className="w-4 h-4 md:hidden" />
                          Local Context Source
                        </span>
                        <span className="text-xs bg-slate-800 text-slate-400 px-2 py-1 rounded border border-slate-700 shrink-0 ml-2">
                          {sourceData.region}
                        </span>
                      </div>

                      {/* 👇 APPLIED "TINOS" (TIMES NEW ROMAN) HERE */}
                      <div className="relative pl-4 border-l-2 border-slate-700 mb-4">
                        <p
                          className={`text-sm md:text-base text-slate-200 text-justify leading-relaxed ${serifFont.className}`}
                        >
                          "{sourceData.content}"
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-3 pt-3 border-t border-slate-800/50 w-full">
                        <span className="text-xs text-slate-500 font-semibold uppercase shrink-0">
                          Reference:
                        </span>
                        {sourceData.isUrl ? (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1.5 text-xs text-blue-400 hover:text-blue-300 hover:underline transition-colors min-w-0 flex-1 group/link"
                          >
                          ...
                          <span className="inline-flex items-center gap-1.5 text-xs text-slate-400 min-w-0 flex-1">
